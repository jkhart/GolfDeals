class DealsController < ApplicationController
  ...
  def index
    ...
    @golf_courses = @golf_courses.having("deals.tee_time > '#{Time.zone.now.to_s(:db)}'")
    @golf_courses = @golf_courses.having("deals.percentage_savings >= ?", params[:savings]) if params[:savings].present?
    @golf_courses = @golf_courses.having("deals.cost_per_player >= ?", params[:minimum_cost]) if params[:minimum_cost].present?
    @golf_courses = @golf_courses.having("deals.cost_per_player <= ?", params[:maximum_cost]) if params[:maximum_cost].present?
    @golf_courses = @golf_courses.having("deals.maximum_players <= ? AND deals.minimum_players >= ?", params[:players], params[:players]) if params[:players].present?
    @golf_courses = @golf_courses.having("deals.includes_cart" => true) if params[:cart].present?
